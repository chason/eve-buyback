...
- **Boots then exits with "BUYBACK_TOKEN_ENCRYPTION_KEY is not a valid Fernet
  key":** the key is validated at startup — it must be an actual **Fernet** key
  (44 chars, ends `=`), generated with the command in step 5. A generic random
  secret (e.g. a `token_urlsafe` value like the session secret) is not a valid
  Fernet key. Unset the variable entirely if you don't price at player structures.
