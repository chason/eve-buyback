...
async def refresh_roster(
    ...
    *,
    ...
    `StructureTokenMissing` if no token is connected (via `get_structure_access_token`) and
    ...
    # Refreshes the access token server-side; raises StructureTokenMissing/Expired.
